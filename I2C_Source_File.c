...
void I2C_Start_Wait(char slave_write_address)
{
  ...
  {   
    ...
    if(SSPCON2bits.ACKSTAT)                     /* Check whether Acknowledgment received or not */
    {
...
char I2C_Repeated_Start(char slave_read_address)
{
    ...
    if (SSPCON2bits.ACKSTAT)                    /* Return 2 if acknowledgment received else 1 */
        return 1;
    else
        return 2;
}
...
void I2C_Nack()
{
    SSPCON2bits.ACKDT = 1;          			/* Not Acknowledge data 1:NACK,0:ACK */
	SSPCON2bits.ACKEN = 1;              		/* Enable ACK to send */	          
    while(SSPCON2bits.ACKEN);
}
...
char I2C_Stop()
{
    ...
    while(SSPCON2bits.PEN);                     /* Wait for end of STOP condition */
    SSPIF = 0;
